...
device.on('connect', function() {
    ...
    // subscribling to 'scalable/heart-beat-sensor/dump' for notficiation about which node to dump too.
    device.subscribe(scalable + deviceName + dump);
    // subscribling to 'scalable/heart-beat-sensor/dump/receive' for notficiation about being the dump node.
...
function getSensorData(deviceName) {
    let message = {
        'respiratoryMonitorSensor' : randomIntBetween(11,32) // 15- 20 normal .. less than 12 is called bradypnea and greater than 30 are called tachypnea
        // use case for systole and diastole being high and low
    };

    
    const device_data = { 
        'respiratoryMonitorSensor': {
