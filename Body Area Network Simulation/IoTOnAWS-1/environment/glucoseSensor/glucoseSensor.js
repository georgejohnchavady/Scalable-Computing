...
device.on('connect', function() {
    ...
    // subscribing to 'scalable/sink/insulin-sensor' for charger notifications.
    device.subscribe(sinkTopic + deviceName);
    // subscribling to 'scalable/insulin-sensor/dump' for notficiation about which node to dump too.
    device.subscribe(scalable + deviceName + dump);
    // subscribling to 'scalable/insulin-sensor/dump/receive' for notficiation about being the dump node.
...
function infiniteLoopPublish() {
    ...
    if(status == 'dead') {
        ...
        if(status == 'active' || status == 'awake') {
            console.log('Sending sensor telemetry data to Sink for ' + deviceName);
        }
...
function getSensorData(deviceName) {
    let message = {
        'glucoseLevel': randomFloatBetween(3.5, 8.5)// more than 8 and leas than 4 dangerous - normal is 4 - 5.4
    };
